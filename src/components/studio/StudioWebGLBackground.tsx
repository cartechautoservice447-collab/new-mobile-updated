...
export const StudioWebGLBackground: React.FC<StudioWebGLBackgroundProps> = ({
  ...
  useEffect(() => {
    ...
    const renderLoop = (time: number) => {
      ...
      // In modal mode, uRenderBg is 0.0 so area outside modal glass is transparent,
      // letting the softened underlying dashboard show through, while inside modal glass
      // it refracts the live, sharp 3D background orbs!
      threeRef.current.material.uniforms.uRenderBg.value = isModal ? 0.0 : (isOrb ? 1.0 : 0.0);
