...
export const CourseDetailModal: React.FC<CourseDetailModalProps> = ({
  ...
  const [activeView, setActiveView] = useState<'hub' | 'notes' | 'collections' | 'overview'>('hub');
  const [notesViewMode, setNotesViewMode] = useState<'grid' | 'reader'>('grid');
  ...
  }, [activeView, showHistory, isOpen, notesViewMode, selectedNote]);

  if (!isOpen || !course) return null;

  const totalNotes = course.noteCount || course.notes.length || 0;
  // Liquid Glass surface styling matching Studio Dashboard: transparent background and no dark drop-shadow masking
  // so the underlying WebGL optical refraction, chromatic dispersion, and specular rims are fully revealed.
  const glassCardStyle = {
    background: 'transparent',
    border: isLightBg ? '1px solid rgba(255, 255, 255, 0.28)' : '1px solid rgba(255, 255, 255, 0.18)',
    boxShadow: 'none',
  };
  ...
  const uniqueCourseTags = new Set(course.notes.flatMap((n) => n.tags || [])).size;
  const totalEstMinutes = course.notes.reduce((acc, n) => {
  ...
  return (
    ...
    >
      {/* Transparent Click-Catcher Layer: Subtle modal dimming and softening are managed at z-40 behind the WebGL glass (z-45) */}
      <div 
      ...
      <div
        ...
        className="relative pointer-events-auto z-10 w-full max-w-6xl xl:max-w-7xl min-h-[660px] lg:min-h-[740px] max-h-[92vh] overflow-y-auto rounded-[36px] sm:rounded-[48px] md:rounded-[56px] p-5 sm:p-8 md:p-10 lg:p-12 text-white flex flex-col justify-between"
      ...
      >
          ...
          <>
            ...
            <div className="space-y-6 sm:space-y-7">
              ...
              <div
                ...
                className="relative w-full rounded-[28px] sm:rounded-[36px] p-5 sm:p-6 md:px-8 md:py-6  flex items-center justify-between gap-4 transition-all"
                style={glassCardStyle}
              >
              ...
              <div
                ...
                className="relative w-full rounded-[24px] sm:rounded-[30px] p-5 sm:p-6 md:px-8 md:py-5  mt-5 sm:mt-6 transition-all"
                style={glassCardStyle}
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div
                  ...
                  className="relative overflow-hidden p-6 sm:p-7 rounded-[30px] sm:rounded-[36px] hover:border-white/35 transition-all duration-300 cursor-pointer group flex flex-col justify-between min-h-[160px] sm:min-h-[175px]"
                  style={glassCardStyle}
                >
        ...
        {activeView === 'notes' && (
          /* Notes Reader & All Notes Glass Grid View */
          <div className="flex-1 flex flex-col h-full overflow-hidden">
            <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-4 gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative w-44 sm:w-60">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-7 py-1.5 rounded-xl bg-white/5 border border-white/10 text-xs text-white placeholder-slate-400 focus:outline-none focus:border-cyan-400"
                  />
                  {searchQuery && (
                    <button
            ...
            {notesViewMode === 'grid' ? (
              <div className="flex-1 overflow-y-auto pr-1">
                {filteredNotes.length === 0 ? (
                  <div className="p-12 text-center text-slate-400 text-xs flex flex-col items-center justify-center">
                    <FileText className="w-10 h-10 text-slate-500 mb-3 stroke-1" />
                    <p className="text-sm font-semibold text-slate-300">No notes found</p>
                    <p className="text-xs text-slate-400 mt-1">
                      {course.notes.length === 0
                        ? 'No notes stored in this course yet.'
                        : 'No notes match your search query.'}
                    </p>
                    {searchQuery && (
                      <button
                        onClick={() => setSearchQuery('')}
                        className="mt-4 px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/15 text-xs text-white border border-white/15 cursor-pointer"
                      >
                        Clear Search
                      </button>
                    )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 pb-4">
                    {filteredNotes.map((note, index) => (
                      <div
                        id={`course-note-card-${note.id}`}
                        key={note.id}
                        onClick={() => {
                          setSelectedNote(note);
                          setNotesViewMode('reader');
                        }}
                        className="relative overflow-hidden p-5 sm:p-6 rounded-[28px] sm:rounded-[32px] hover:border-white/35 transition-all duration-300 cursor-pointer group flex flex-col justify-between min-h-[175px]"
                        style={glassCardStyle}
                      >
                        <div>
                          <div className="flex items-center justify-between mb-3">
                          ...
                          <h4 className="text-base sm:text-lg font-extrabold text-white tracking-tight group-hover:text-cyan-300 transition-colors line-clamp-1">
                            {note.title}
                          </h4>
                          <p className="text-xs sm:text-sm text-slate-300/80 mt-2 line-clamp-2 leading-relaxed">
                            {note.summary}
                          </p>
                        </div>
                        ...
                        <div className="mt-4 pt-3 border-t border-white/10 flex items-center justify-between">
                          <div className="flex flex-wrap gap-1.5">
                            {note.tags.map((tag, ti) => (
                              <span
                                key={ti}
                                className="px-2 py-0.5 rounded-md text-[10px] font-medium bg-cyan-500/15 border border-cyan-400/25 text-cyan-300"
                              >
                                {tag}
                              </span>
                            ))}
                          </div>
                          <div className="flex items-center gap-1 text-xs font-semibold text-cyan-300 group-hover:text-white transition-colors">
                            <span>Read</span>
              ...
              <div className="flex-1 flex flex-col md:flex-row gap-4 overflow-hidden">
                {/* Notes Sidebar with WebGL liquid glass for each note item */}
                <div className="w-full md:w-80 flex flex-col max-h-48 md:max-h-none shrink-0 overflow-y-auto space-y-2.5 pr-1">
                  {filteredNotes.map((note) => {
                    const isSelected = selectedNote?.id === note.id;
                    return (
                      <div
                        id={`course-note-item-${note.id}`}
                        key={note.id}
                        onClick={() => setSelectedNote(note)}
                        className={`relative overflow-hidden p-3.5 rounded-2xl transition-all cursor-pointer flex items-start gap-2.5 ${
                          isSelected ? 'border-cyan-400/50' : 'hover:border-white/30'
                        }`}
                        style={glassCardStyle}
                      >
                        <FileText
                ...
                {/* Full Note Reader Body in WebGL liquid glass panel */}
                <div
                  id="course-note-reader-glass-panel"
                  className="flex-1 flex flex-col overflow-y-auto p-5 sm:p-7 rounded-[28px] sm:rounded-[32px] space-y-4 sm:space-y-6"
                  style={glassCardStyle}
                >
                  {selectedNote ? (
                    <>
                      <div className="space-y-3 pb-4 border-b border-white/10">
                        <div className="flex items-center justify-between flex-wrap gap-2">
                          ...
                          <button
                            onClick={() => setNotesViewMode('grid')}
                            className="flex items-center gap-1.5 text-xs text-cyan-300 hover:text-white px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 transition-all cursor-pointer"
                          >
                            <LayoutGrid className="w-3.5 h-3.5" />
                            <span>View All Notes Cards</span>
                          </button>
                        </div>
        ...
        {activeView === 'overview' && (
          /* Course Overview WebGL Liquid Glass Interface */
          <div className="flex-1 flex flex-col h-full overflow-y-auto pr-1 animate-fade-in space-y-6">
            {/* Top Navigation & Status Bar */}
            <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-2 gap-3 flex-wrap">
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setActiveView('hub')}
                  className="flex items-center gap-2 text-xs font-semibold px-3.5 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-white border border-white/15 cursor-pointer transition-all active:scale-95 shadow-sm"
                >
                  <ArrowLeft className="w-4 h-4" />
                  <span>Back to Course Hub</span>
                </button>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-mono font-bold text-cyan-400 px-2 py-0.5 rounded-md bg-cyan-500/10 border border-cyan-400/20">
            ...
            {/* 4 Key Metric Glass Cards */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
              <div
                id="course-overview-metric-1"
                className="p-5 rounded-2xl sm:rounded-3xl border border-white/15 space-y-2 hover:border-white/30 transition-all duration-300"
                style={glassCardStyle}
              >
                <div className="flex items-center gap-2 text-blue-400 text-xs font-semibold">
                  <BookOpen className="w-4 h-4" /> Course Notes
                </div>
                <p className="text-2xl sm:text-3xl font-bold text-white">{course.notes.length}</p>
                <p className="text-[11px] text-slate-400 truncate">
                  {uniqueCourseTags > 0 ? `${uniqueCourseTags} topic collections` : 'All modules documented'}
                </p>
              </div>

              <div
                id="course-overview-metric-2"
                className="p-5 rounded-2xl sm:rounded-3xl border border-white/15 space-y-2 hover:border-white/30 transition-all duration-300"
                style={glassCardStyle}
              >
                ...
                <p className="text-2xl sm:text-3xl font-bold text-white">{course.progress}%</p>
                <p className="text-[11px] text-emerald-400 font-medium truncate">
                  {course.progress >= 80 ? 'Mastery phase' : course.progress >= 50 ? 'Intermediate phase' : `${100 - course.progress}% remaining`}
                </p>
              </div>

              <div
                id="course-overview-metric-3"
                className="p-5 rounded-2xl sm:rounded-3xl border border-white/15 space-y-2 hover:border-white/30 transition-all duration-300"
                style={glassCardStyle}
              >
                <div className="flex items-center gap-2 text-purple-400 text-xs font-semibold">
                  <Clock className="w-4 h-4" /> Est. Reading
                </div>
                <p className="text-2xl sm:text-3xl font-bold text-white">
                  {totalEstMinutes > 0 ? `${totalEstMinutes}m` : '—'}
                </p>
                <p className="text-[11px] text-slate-400 truncate">
                  {totalEstMinutes > 0 ? `Across ${course.notes.length} lectures` : 'No read-time recorded'}
                </p>
              </div>

              <div
                id="course-overview-metric-4"
                className="p-5 rounded-2xl sm:rounded-3xl border border-white/15 space-y-2 hover:border-white/30 transition-all duration-300"
                style={glassCardStyle}
              >
                <div className="flex items-center gap-2 text-amber-400 text-xs font-semibold">
                  <Award className="w-4 h-4" /> Instructor
                </div>
                <p className="text-lg sm:text-xl font-bold text-white truncate" title={course.instructor}>
                  {course.instructor || 'Faculty Staff'}
                </p>
                <p className="text-[11px] text-slate-400 truncate">
            ...
            {/* Syllabus & Lecture Progress Panel */}
            <div
              id="course-overview-syllabus-panel"
              className="p-5 sm:p-7 rounded-2xl sm:rounded-3xl border border-white/15 space-y-5"
              style={glassCardStyle}
            >
              <div className="flex items-center justify-between">
                <h3 className="text-base font-bold text-white flex items-center gap-2.5">
                  <TrendingUp className="w-5 h-5 text-cyan-400" />
                  <span>Syllabus & Lecture Progress</span>
                </h3>
                <span className="text-xs text-slate-400 font-mono">
                  {course.notes.length} modules registered
                </span>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-medium text-slate-300">Overall Course Mastery</span>
                ...
                <div className="w-full h-2.5 rounded-full bg-white/10 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${course.progress}%`,
                      backgroundColor: course.color || '#10b981',
                      boxShadow: `0 0 12px ${course.color || '#10b981'}80`,
                    }}
                  />
                </div>
              </div>
              ...
              {course.notes.length > 0 ? (
                <div className="pt-3 border-t border-white/10 space-y-2.5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 max-h-60 overflow-y-auto pr-1">
                    {course.notes.map((note, idx) => {
                      const isCompleted = idx < Math.ceil((course.progress / 100) * course.notes.length);
                      return (
                        <div
                          key={note.id || idx}
                          onClick={() => {
                          ...
                          className="flex items-center justify-between p-3 rounded-2xl bg-white/[0.04] border border-white/10 text-xs hover:bg-white/10 hover:border-cyan-400/30 cursor-pointer transition-all group"
                        >
                ...
                <div className="p-4 rounded-xl bg-white/[0.04] text-center text-xs text-slate-400">
                  No notes recorded yet for this course. Start study sessions or add notes from the course hub.
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div
                id="course-overview-action-1"
                onClick={() => {
                  if (onOpenPomodoro) {
                    onOpenPomodoro();
                  }
                }}
                className="p-5 rounded-2xl sm:rounded-3xl border border-white/15 hover:border-cyan-400/50 cursor-pointer transition-all hover:scale-[1.01] flex items-center justify-between group"
                style={glassCardStyle}
              >
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2 text-cyan-300 text-xs font-semibold">
                    <Play className="w-4 h-4 text-cyan-400 fill-cyan-400/20" /> Study Session
                  </div>
                  <p className="text-sm sm:text-base font-bold text-white group-hover:text-cyan-300 transition-colors">
                    Launch Focus Timer
                  </p>
                  <p className="text-xs text-slate-400">
                    25-minute Pomodoro timer for {course.code}
                  </p>
                </div>
                <span className="px-3.5 py-1.5 text-xs font-semibold rounded-xl bg-cyan-500/20 text-cyan-300 border border-cyan-400/30 group-hover:bg-cyan-500/30 transition-all">
                  Launch →
                </span>
              </div>

              <div
                id="course-overview-action-2"
                onClick={() => {
                  setActiveView('notes');
                  setNotesViewMode('grid');
                }}
                className="p-5 rounded-2xl sm:rounded-3xl border border-white/15 hover:border-emerald-400/50 cursor-pointer transition-all hover:scale-[1.01] flex items-center justify-between group"
                style={glassCardStyle}
              >
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2 text-emerald-300 text-xs font-semibold">
                    <FileText className="w-4 h-4 text-emerald-400" /> Course Notes
                  </div>
                  <p className="text-sm sm:text-base font-bold text-white group-hover:text-emerald-300 transition-colors">
                    Explore All Notes
                  </p>
                  <p className="text-xs text-slate-400">
                    Browse all {course.notes.length} note cards and reader
                  </p>
                </div>
                <span className="px-3.5 py-1.5 text-xs font-semibold rounded-xl bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 group-hover:bg-emerald-500/30 transition-all">
                  Browse →
                </span>
              </div>
            </div>

            {/* Course Milestones Panel */}
            <div
              id="course-overview-milestones-panel"
              className="p-5 sm:p-6 rounded-2xl sm:rounded-3xl border border-white/15 space-y-3"
              style={glassCardStyle}
            >
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                {course.code} Learning Milestones
              </h4>
              <div className="space-y-2.5 text-xs sm:text-sm">
                ...
                <div className="flex items-center gap-2.5 text-emerald-300">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                  <span>Review syllabus topics & key concepts with {course.instructor || 'instructor'}</span>
                </div>
