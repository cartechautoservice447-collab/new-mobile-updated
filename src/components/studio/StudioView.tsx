...
export const StudioView: React.FC<StudioViewProps> = ({
  ...
  const measureAndCacheBoxes = useCallback(() => {
    ...
    if (selectedCourseRef.current) {
      // COURSE MODAL OPEN: Switch WebGL box set strictly to the Course modal boxes.
      // Hidden dashboard boxes must NOT compete with active Course boxes.
      ...
      // All Notes interface: Register every note card, sidebar note item, and the note reader panel
      if (selectedCourseRef.current.notes) {
        selectedCourseRef.current.notes.forEach((note) => {
          pushBoxById(`course-note-card-${note.id}`, 28, 42);
          pushBoxById(`course-note-item-${note.id}`, 18, 28);
        });
      }
      pushBoxById('course-note-reader-glass-panel', 28, 42);

      // Course Overview interface: Register metric cards, syllabus panel, actions, and milestones
      pushBoxById('course-overview-metric-1', 24, 38);
      pushBoxById('course-overview-metric-2', 24, 38);
      pushBoxById('course-overview-metric-3', 24, 38);
      pushBoxById('course-overview-metric-4', 24, 38);
      pushBoxById('course-overview-syllabus-panel', 28, 42);
      pushBoxById('course-overview-action-1', 28, 42);
      pushBoxById('course-overview-action-2', 28, 42);
      pushBoxById('course-overview-milestones-panel', 28, 42);

      // Also register Overview Modal if opened over the course
      pushBoxById('overview-modal-content', 36, 55);
      pushBoxById('overview-metric-1', 24, 36);
      pushBoxById('overview-metric-2', 24, 36);
      pushBoxById('overview-metric-3', 24, 36);
      pushBoxById('overview-metric-4', 24, 36);
      pushBoxById('overview-syllabus-panel', 28, 40);
      pushBoxById('overview-action-1', 28, 40);
      pushBoxById('overview-action-2', 28, 40);
      pushBoxById('overview-milestones-panel', 28, 40);
    } else {
      // NORMAL DASHBOARD: dashboard boxes participate normally
      ...
      // Studio Header Buttons & Option Controls
      pushBoxById('btn-return-classic-header', 12, 16);
      pushBoxById('btn-ai-sparkle-header', 12, 16);
      pushBoxById('btn-perf-high', 8, 12);
      pushBoxById('btn-perf-ultra', 8, 12);
      pushBoxById('btn-theme-toggle', 12, 16);
      pushBoxById('btn-settings-header', 12, 16);
      pushBoxById('btn-profile-header', 12, 16);

      // Showcase Bar Action & Option Buttons
      pushBoxById('btn-showcase-all-bg', 14, 18);
      SHOWCASE_TEMPLATES.forEach((tmpl) => {
        pushBoxById(`btn-showcase-tmpl-${tmpl.id}`, 18, 22);
      });

      // Course & Timeline Action Buttons
      pushBoxById('btn-courses-view-all', 14, 18);
      pushBoxById('add-new-course-btn', 28, 36);
      courses.forEach((course) => {
        pushBoxById(`btn-course-open-${course.id}`, 12, 16);
        pushBoxById(`btn-course-delete-${course.id}`, 12, 16);
      });
      ['l1', 'l2', 'l3', 'l4'].forEach((lid) => {
        pushBoxById(`btn-timeline-open-${lid}`, 16, 20);
      });

      // Bottom Dock Navigation Buttons
      pushBoxById('btn-dock-home', 20, 24);
      pushBoxById('btn-dock-courses', 20, 24);
      pushBoxById('btn-dock-collections', 20, 24);
      pushBoxById('btn-dock-notes', 20, 24);
      pushBoxById('btn-dock-more', 20, 24);
      pushBoxById('btn-return-classic-dock', 20, 24);

      // Register active Modals as Liquid Glass surfaces if open
  ...
  // When Course modal opens or closes, immediately recalculate cached boxes
  useEffect(() => {
    measureAndCacheBoxes();
  ...
  }, [selectedCourse, measureAndCacheBoxes]);
  ...
  return (
    ...
    >
            ...
            <div className="space-y-4">
              {courses.map((course) => (
                <div
                  ...
                  className="group relative rounded-[24px] p-5 md:p-6 flex items-center justify-between cursor-pointer transition-all duration-300 hover:scale-[1.01] active:scale-[0.99] bg-white/[0.06] hover:bg-white/[0.12] border border-white/15 hover:border-white/30 shadow-lg"
                >
            ...
            {[
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-2.5 self-start lg:self-auto">
                  ...
                  <button
                    id={`btn-timeline-open-${lecture.id}`}
                    onClick={() => setSelectedCourse(COURSES_DATA[0])}
                ...
                >
                  ...
                  <div className="mt-5 pt-4 border-t border-white/10 flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      {isNewlyCreated && (
                        <button
                          id={`btn-course-delete-${course.id}`}
      ...
      {/* Subtle Modal Softening Backdrop (Layer 2: between Dashboard and Course WebGL Glass) */}
      {selectedCourse && (
        <div
          id="course-modal-dimming-backdrop"
          className="fixed inset-0 z-40 bg-black/30 backdrop-blur-[3px] pointer-events-auto transition-opacity duration-300 animate-fade-in"
